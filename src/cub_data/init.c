...
/*   Updated: 2024/07/29 22:25:07 by akamite          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init_cub_data(t_cub_data *cub_data)
{
	...
	if (!cub_data->mlx)
		free_exit(cub_data, err_msg("mlx_init() Error.", EXIT_FAILURE));

	cub_data->win = mlx_new_window(cub_data->mlx, WIN_WIDTH, WIN_HEIGHT, "Cub3D");
	if (!cub_data->win)
		free_exit(cub_data, err_msg("mlx_new_window() Error.", EXIT_FAILURE));
}
